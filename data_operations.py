...
# from shapely.geometry import Point, Polygon #Enabling may cause user registration issues
...
class DatabaseManagement:
    ...
    def create_user(self, user_name, email_id, phone_number, location) -> str:
        """
        ...
        :return:
        """
        # user: User = User(user_name, email_id, phone_number, location=Point([(0, 0)])) #Added Location by Suraj Point([(0, 0)]))
        user: User = User(user_name, email_id, phone_number,
                          location=location)  # Added Location by Suraj Point([(0, 0)]))
    ...
    # Added by Suraj
    def create_multiple_users(self, count, name, email, phone, location):
        self.fake = Faker('en_IN')
        lat_min, lat_max = 77.491, 77.861
        lon_min, lon_max = 12.834, 13.139
        users = [
            {
                "user_name": name if name else self.fake.name(),
                "email_id": email if email else self.fake.email(),
                "phone_number": phone if phone else self.fake.phone_number(),
                # "location": {"type": "Point", "coordinates": (0, 0)}
                ...
                "location": {"type": "Point", "coordinates": location if location else (round(random.uniform(lat_min, lat_max), 3), round(random.uniform(lon_min, lon_max), 3))},
    ...
    # Added by Suraj to check if the provided location are within the operations boundary.
    def taxi_within_boundary(self, city: str, latitude: float, longitude: float):
        """
        Works for both USER and TAXI
        Book a taxi within the city boundary
        @param city:
        @param latitude:
        @param longitude:
        @return: Boolean indicating if the location is within the boundary
        """
        ...
        if (left_bottom[0] <= latitude <= left_top[0] and
            ...
            raise ValueError(f"Location is outside the operation boundary", boundary)
    ...
    def find_nearest_taxi(self, current_location,
        ...
        @return:
        """
        latitude = current_location["coordinates"][0]
        longitude = current_location["coordinates"][1]
